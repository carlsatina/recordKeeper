...
<div class="vital-entry-page">
        ...
        <div v-else-if="record" class="detail-stack">
            <section class="card measurement-card sugar glass-card">
                <div class="measurement-header">
                    ...
                    <div>
                        <p class="measurement-label">{{ record?.chartGroup || 'Blood Sugar' }}</p>
                        <p class="measurement-status">mg/dL</p>
                    </div>
                </div>
                <div v-if="!isEditing" class="measurement-display">
                    <p class="measurement-value">{{ displayReading }}</p>
                    <p class="measurement-subtitle">{{ record?.chartGroup || 'Fasting' }}</p>
                </div>
                <div v-else class="form-grid">
                    <div class="form-group">
                        <label>Reading (mg/dL)</label>
                        <input type="number" v-model="form.reading" placeholder="95"/>
                    </div>
                    <div class="form-group">
                        <label>Context</label>
    ...
    <div v-if="showDeleteConfirm" class="confirm-overlay">
        <div class="confirm-card">
            ...
            <p class="confirm-text">This will permanently remove this blood sugar entry.</p>
...
export default {
    ...
    setup() {
        ...
        const { fetchRecordById, updateRecord, deleteRecord } = useBloodSugar()
        ...
        const populateForm = () => {
            ...
            form.reading = record.value.valueNumber ?? ''
            form.context = record.value.chartGroup || 'fasting'
        ...
        const saveChanges = async () => {
            if (!record.value) return
            if (!form.reading) {
                formError.value = 'Reading is required.'
                return
            }
            const token = localStorage.getItem('token')
            if (!token) {
        ...
        const displayReading = computed(() => record.value?.valueNumber ?? '--')
